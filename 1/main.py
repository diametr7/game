...
def info():
    # rules
    intro_text = ["Правила игры:", "Во время игры появляются корабли, которые надо взрывать с помощью бомб. ",
                  "Бомба выпускается по щелчку мыши. Игра содеражит уровни. ",
                  "Во время игры корабли могут становиться черными и фиолетовыми. В черный корабль ",
                  "можно попасть и убить его, при попадании в фиолетовый корабль ничего не происходит.",
                  "Если вы подобьете 4 корабля, то перейдете на новый уровень и все ",
                  "неиспользованные бомбы перейдут в запас (но в запасе не может быть ",
                  "больше 5 бомб). Если же вы подобьете меньше 4 кораблей, то игра закончится.",
                  "В финальном окне вы сможете ввести свое имя, узнать свое место в рейтинге, ",
                  "узнать лидеров, начать новую игру или выйти."]
    screen.fill((30, 30, 50))
...
class Sheep(pygame.sprite.Sprite):
    ...
    def __init__(self, x, n, log):
        super().__init__(sheep_sprites)
        self.image = load_image('1.png', color_key=-1)
        self.go = 1
        self.n = n
        self.rect = self.image.get_rect()
        self.rect.x = x
    ...
    def revers(self):
        self.go = (self.go + 1) % 2
        self.image = load_image(str(self.go) + '.png', color_key=-1)
    ...
    def update(self, *args):
        if self.rect.x <= -200:
            ...
            self.x -= self.v
            self.rect.x = int(self.x)
        if self.rect.x <= WIDTH + 200 - self.n * parts and self.log == 1:
            if self.n != N:
                self.n += 1
                self.revers()
    ...
    def fired(self):
        ...
        sheep_killed_last += 1
        if len(unsheep_sprites) + len(sheep_sprites) >= 2:
            UnSheep('1.png', self.rect.x, self.n)
        else:
            Sheep(WIDTH, 1, randint(0, 1))
...
def main():
    ...
    while running:
        ...
        if bomb_pysked_last == 5 + saved_bombs and drawing is False:
            ...
            change_level()
            middle_screen()
        pygame.draw.rect(screen, (0, 0, 0), (0, 0, 800, 60))
        pr_line(f"Осталось {5 + saved_bombs - bomb_num} бомб", 130, 10, 'white')
        pr_line(f"{level} уровень", 10, 9, 'white')
        pr_line(f"Убито {sheep_killed_last} кораблей", 330, 10, 'white')
